...
void TinySCF_compute_sq_Schwarz_scrvals(TinySCF_t TinySCF)
{
    ...
    #pragma omp parallel
    {
        ...
        for (int M = 0; M < nshells; M++)
        {
            ...
            for (int N = 0; N < nshells; N++)
            {
                ...
                TinySCF->sp_scrval[M * nshells + N] = maxval;
    ...
    int df_nshells = TinySCF->df_nshells;
    double global_max_df_scrval = 0;
    for (int i = 0; i < df_nshells; i++)
    {
    ...
    for (int M = 0; M < nshells; M++)
    {
        for (int N = 0; N < nshells; N++)
        {
            double sp_scrval = TinySCF->sp_scrval[M * nshells + N];
...
void TinySCF_prepare_sparsity(TinySCF_t TinySCF)
{
    ...
    if (my_rank == 0)
    {
        ...
        printf("TinySCF rank 0 regular + density fitting memory usage = %.2lf MB \n", TinySCF->mem_size / 1048576.0);
...
static void TinySCF_calc_energy(TinySCF_t TinySCF)
{
    ...
    MPI_Bcast(&TinySCF->HF_energy, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
}
...
void TinySCF_do_SCF(TinySCF_t TinySCF)
{
    ...
    if (my_rank == 0) printf("TinySCF SCF iteration started...\n");
    if (my_rank == 0) printf("Nuclear energy = %.10lf\n", TinySCF->nuc_energy);
    TinySCF->iter = 0;
    ...
    while ((TinySCF->iter < TinySCF->niters) && (energy_delta >= TinySCF->ene_tol))
    {
        ...
        et1 = get_wtime_sec();
        if (my_rank == 0) printf("* Build Fock matrix     : %.3lf (s)\n", et1 - st1);

        // Calculate new system energy
        st1 = get_wtime_sec();
        TinySCF_calc_energy(TinySCF);
        et1 = get_wtime_sec();
        if (my_rank == 0) printf("* Calculate energy      : %.3lf (s)\n", et1 - st1);
        ...
        et1 = get_wtime_sec();
        if (my_rank == 0) printf("* DIIS procedure        : %.3lf (s)\n", et1 - st1);
        
        // Diagonalize and build the density matrix
        st1 = get_wtime_sec();
        TinySCF_build_DenMat(TinySCF);
        et1 = get_wtime_sec();
        if (my_rank == 0) printf("* Build density matrix  : %.3lf (s)\n", et1 - st1);
        
        et0 = get_wtime_sec();
        
        if (my_rank == 0) printf("* Iteration runtime     = %.3lf (s)\n", et0 - st0);
        if (my_rank == 0) printf("* Energy = %.10lf (%.10lf)", TinySCF->HF_energy + TinySCF->nuc_energy, TinySCF->HF_energy);
        if (TinySCF->iter > 0) 
        {
            if (my_rank == 0) printf(", delta = %e\n", energy_delta); 
        } else {
            if (my_rank == 0) printf("\n");
